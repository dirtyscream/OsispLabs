...
int enqueue(Message *msg) {
    if (sem_wait(&empty) != 0) {
        if (errno == EINTR && terminate_flag) return -1;
        perror("sem_wait empty in enqueue");
        return -1;
    }
    if (terminate_flag) {
        sem_post(&empty);
...
Message *dequeue() {
    if (sem_wait(&full) != 0) {
        if (errno == EINTR && terminate_flag) return NULL;
        perror("sem_wait full in dequeue");
        return NULL;
    }
    if (terminate_flag) {
        sem_post(&full);
...
void *consumer_thread(void *arg) {
    ...
    printf("Consumer thread %d exiting.\n", thread_id);
    pthread_exit(NULL);
}
...
void decrease_queue_size() {
    ...
    if (sem_trywait(&empty) == -1) {
        if (errno == EAGAIN) {
            printf("Cannot decrease queue size now (no free slots to reserve).\n");
        } else {
            perror("sem_trywait on decrease");
        }
        pthread_mutex_unlock(&mutex);
        return;
    }
...
int main() {
    ...
    if (sem_init(&empty, 0, (unsigned int)queue.max_size) != 0) { perror("Sem 'empty' init"); pthread_mutex_destroy(&mutex); free(queue.buffer); return 1; }
    if (sem_init(&full, 0, 0) != 0) { perror("Sem 'full' init"); sem_destroy(&empty); pthread_mutex_destroy(&mutex); free(queue.buffer); return 1; }

    signal(SIGINT, signal_handler);

    printf("Main process started (Lab 5.1 - Semaphores).\n");
